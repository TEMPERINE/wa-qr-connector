...
function toMessageDTO(msg) {
  return {
    ...
    mentionedIds: safeDTO(msg?._data?.mentionedJidList) ?? [],
  };
}
...
function getOrCreateSession(tenant) {
  ...
  const setStatus = (st) => {
    if (s.status === st) return; // evita flood
    s.status = st;
  ...
  // --- eventos em tempo real (SSE global e incremental) ---
  ...
  client.on("message_create", (msg) => {
    // garante evento para mensagens do próprio usuário também
    broadcast(tenant, { type: "message_create", message: toMessageDTO(msg) });
  });
...
// Health-check GLOBAL (para monitor/Lovable)
...
// 1) Listar chats com filtros/paginação e atualização incremental opcional
// GET /sessions/:tenant/chats?q=&isGroup=true|false&archived=true|false&unreadOnly=true|false&limit=30&offset=0&updatedSince=TIMESTAMP
app.get("/sessions/:tenant/chats", async (req, res, next) => {
  try {
    const { tenant } = req.params;
    const q = (req.query.q || "").toString().toLowerCase();
    const limit = Math.max(1, Ma
