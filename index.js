...
async function enrichMessageDTO(msg, client, cache) {
  ...
  // Em grupos, quando houver "author" (remetente), resolvemos o nome
  if (dto.author) {
...
async function toChatDTO(chat, client, cache) {
  ...
  if (last) {
    ...
    if (lastDTO.author) {
      ...
      lastDTO.senderShort = (resolvedName?.split?.(" ")[0] ?? resolvedName) || null;
    }
  }
...
app.post("/sessions/:tenant/messages/media", async (req, res, next) => {
  try {
    ...
    let media;
    if (mediaUrl) {
      // em Node 18+ fetch global existe; se não, adicione node-fetch
